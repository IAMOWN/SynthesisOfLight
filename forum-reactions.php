    ...
    public function render_reactions_summary($post_id) {
        ...
        if (!empty($reaction_names)) {
            ...
            if (count($reaction_names) === 1) {
                $output .= sprintf(__('%s has responded to this post.', 'asgaros-forum'), $reaction_names[0]);
            } else if (count($reaction_names) === 2) {
                $output .= sprintf(__('%s and %s have responded to this post.', 'asgaros-forum'), $reaction_names[0], $reaction_names[1]);
            } else if (count($reaction_names) === 3) {
                $output .= sprintf(__('%s, %s and %s have responded to this post.', 'asgaros-forum'), $reaction_names[0], $reaction_names[1], $reaction_names[2]);
            } else {
                $output .= sprintf(__('%s, %s and %s other users have responded to this post.', 'asgaros-forum'), $reaction_names[0], $reaction_names[1], count($reaction_names) - 2);
            }
